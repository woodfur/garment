          ...
          <div className="ward-gallery">
            {pieces.map((u) => (
              <div key={u.id} className="card" style={{ padding: 0, overflow: "hidden", opacity: u.is_archived ? 0.65 : 1, transition: "opacity 0.15s" }}>
                <div style={{ height: 180, background: u.bg_removed ? "repeating-conic-gradient(#e9e3d7 0% 25%, #fbf9f4 0% 50%) 0 0 / 20px 20px" : "var(--color-bg-elevated)", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
                  {u.image_url ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              <div style={{ display: "flex", gap: "0.625rem" }}>
                <button onClick={handleUpload} disabled={uploading} className="btn-primary" style={{ flex: 1, padding: "0.75rem" }}>
                  {uploading ? <><Loader2 size={15} className="animate-spin" /> Uploading…</> : <><Upload size={15} /> Upload piece</>}
                </button>
