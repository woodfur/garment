...
const navItems = [
  { href: "/admin/dashboard", label: "Dashboard", ix: "i" },
  { href: "/admin/branches", label: "Branches", ix: "ii" },
];
...
export default function AdminSidebar() {
  ...
  return (
    <aside style={{
      ...
      {/* Brand */}
      <Link href="/admin/dashboard" style={{ textDecoration: "none", display: "flex", alignItems: "center", gap: "0.7rem", color: "inherit" }}>
        <div style={{
          width: 34, height: 34, flexShrink: 0,
          background: "var(--color-primary-dark)", borderRadius: "9px",
          display: "grid", placeItems: "center", color: "#fff",
          fontFamily: "var(--font-heading)", fontSize: "1.125rem",
          boxShadow: "0 8px 18px -8px rgba(71,39,67,0.6)",
        }}>G</div>
        <div>
          <div style={{ fontFamily: "var(--font-heading)", fontWeight: 400, fontSize: "1.3rem", letterSpacing: "-0.01em", lineHeight: 1 }}>Garment</div>
          <div style={{ fontSize: "0.53rem", letterSpacing: "0.3em", textTransform: "uppercase", color: "var(--color-text-faint)", marginTop: 3 }}>Super Admin</div>
        </div>
      </Link>
      ...
      <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2 }}>
        ...
        {navItems.map(({ href, label, ix }) => {
          ...
          return (
            <Link key={href} href={href} prefetch={true} id={`admin-nav-${label.toLowerCase()}`} className={`rail-link${active ? " on" : ""}`}>
      ...
      {/* Sign out */}
      <div style={{ borderTop: "1px solid var(--color-border)", paddingTop: "1.1rem" }}>
        <button
          id="admin-signout"
